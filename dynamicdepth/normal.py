...
os.environ["WANDB_API_KEY"] = '45bc460c10737384258b507a85a7a228f6c90ae6'
...
class Trainer:
	def __init__(self, options):
		...
		# --------------------------SC-depthv3 modules--------------------------------
		self.calculate_normal_ranking_loss = EdgeguidedNormalRankingLoss().to(self.device)
		# ----------------------------------------------------------------------------
	...
	def compute_losses(self, inputs, outputs, is_multi=False):
		...
		for scale in self.opt.scales:
			...
			loss += reprojection_loss + consistency_loss + normal_l1_loss
...
# --------------------------------------------SC-depthV3 modules--------------------------------------------	
	def photo_and_geometry_loss(self, tgt_img, ref_imgs, tgt_depth, ref_depths, intrinsics, poses, poses_inv):
		...
		for ref_img, ref_depth, pose, pose_inv in zip(ref_imgs, ref_depths, poses, poses_inv):
			diff_img_tmp1, diff_color_tmp1, diff_depth_tmp1, valid_mask_tmp1 = self.compute_pairwise_loss(
				tgt_img, ref_img, tgt_depth,
				ref_depth, pose, intrinsics
			)
			diff_img_tmp2, diff_color_tmp2, diff_depth_tmp2, valid_mask_tmp2 = self.compute_pairwise_loss(
	...
	def compute_pairwise_loss(self, tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic):
		...
		ref_img_warped, projected_depth, computed_depth = self.inverse_warp(
		...
		if True:
			ssim_map = self.compute_ssim_loss(tgt_img, ref_img_warped)
	...
	def inverse_warp(self, img, depth, ref_depth, pose, intrinsics, padding_mode='zeros'):
		"""
		...
		T = self.pose_vec2mat(pose)  # [B,3,4]
	...
	def pose_vec2mat(self, vec, rotation_mode='euler'):
		"""
